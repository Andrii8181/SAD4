...
class TableWidget(QTableWidget):
    ...
    def keyPressEvent(self, event):
        # Ctrl+C / Ctrl+V support via standard shortcuts
        ...
        if event.key() in (Qt.Key_Return, Qt.Key_Enter):
            ...
            if r == self.rowCount() - 1:
                ...
                for cc in range(self.columnCount()):
                    if not self.item(self.rowCount()-1, cc):
                        self.setItem(self.rowCount()-1, cc, QTableWidgetItem(""))
...
class MainWindow(QMainWindow):
    ...
    def _build_menus(self):
        ...
        # тимчасово вимкнені функції (пізніше підключимо)
        self.act_open_excel = QAction("Відкрити Excel…", self)
        self.act_open_excel.setEnabled(False)
        m_file.addAction(self.act_open_excel)
        self.act_save_excel = QAction("Зберегти в Excel…", self)
        self.act_save_excel.setEnabled(False)
        m_file.addAction(self.act_save_excel)
        self.act_export_word = QAction("Експорт у Word…", self)
        self.act_export_word.setEnabled(False)
        m_file.addAction(self.act_export_word)
        ...
        m_data = menubar.addMenu("Дані")
        act_copy = QAction("Копіювати", self)
        act_copy.setShortcut("Ctrl+C")
        act_copy.triggered.connect(self.table.copy_selection_to_clipboard)
        m_data.addAction(act_copy)
        act_paste = QAction("Вставити", self)
        act_paste.setShortcut("Ctrl+V")
        act_paste.triggered.connect(self.table.paste_from_clipboard)
        m_data.addAction(act_paste)
        ...
        act_clear = QAction("Очистити таблицю", self)
        act_clear.triggered.connect(self.clear_table)
        m_edit.addAction(act_clear)

        m_analysis = menubar.addMenu("Аналіз даних")
        self.act_check_normality = QAction("Перевірити нормальність (Шапіро–Вілка)", self)
        self.act_check_normality.setEnabled(False)
        m_analysis.addAction(self.act_check_normality)
        self.act_choose_methods = QAction("Обрати методи аналізу…", self)
        self.act_choose_methods.setEnabled(False)
        m_analysis.addAction(self.act_choose_methods)
        self.act_run_analysis = QAction("Виконати аналіз", self)
        self.act_run_analysis.setEnabled(False)
        m_analysis.addAction(self.act_run_analysis)
    ...
    def _build_toolbar(self):
        ...
        a_add_col = QAction("＋ стовпчик", self)
        a_add_col.triggered.connect(self.add_column_after_current)
        tb.addAction(a_add_col)
        a_del_col = QAction("− стовпчик", self)
        a_del_col.triggered.connect(lambda: (self.table.columnCount() and self.table.currentColumn() >= 0 and self.table.removeColumn(self.table.currentColumn())))
        tb.addAction(a_del_col)
        tb.addSeparator()
        a_copy = QAction("Копіювати", self)
        a_copy.triggered.connect(self.table.copy_selection_to_clipboard)
        tb.addAction(a_copy)
        a_paste = QAction("Вставити", self)
        a_paste.triggered.connect(self.table.paste_from_clipboard)
        tb.addAction(a_paste)
        tb.addSeparator()
        a_rename = QAction("Перейменувати стовпчик", self)
        a_rename.triggered.connect(self.table.rename_current_column)
        tb.addAction(a_rename)
        a_clear = QAction("Очистити", self)
        a_clear.triggered.connect(self.clear_table)
    ...
    def clear_table(self):
        ...
        self.statusBar().showMessage("Таблицю очищено", 2000)
